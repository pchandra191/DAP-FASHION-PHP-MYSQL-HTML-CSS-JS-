 ...
 <?php
	...
	if(isset($_GET["data"]))
    {
	   ...
	   <div class="resultblock" data-tag="a" style="display:block" >
		...
		<?php
   for($b=0;$b<99;$b++){
			$qimpo = $conn->query("SELECT * FROM `i_new_product` WHERE brand = '$b' AND cat='$cat_i' AND m_cat=1  ") or die(mysqli_error($conn)); 	 
							while($fimp1 = $qimpo->fetch_array()){
				?>
	...
	<div class="resultblock" data-tag="a" style="display:block" >
		...
		for($bid=0;$bid<99;$bid++){
			$qimpo = $conn->query("SELECT * FROM `i_new_product` WHERE brand = '$bid' AND m_cat=1   ") or die(mysqli_error($conn)); 	 
							while($fimp1 = $qimpo->fetch_array()){
				?>
					<div class="product-grid love-grid">
						<div class="more-product-info"><span>TOP</span></div>						
...
<!-- SHOW ALL, 20-04-2018, Prabhat Chandra -->
	...
	foreach($brand_id as $brid){	
		...
		<?php 	
			...
			$qimpo = $conn->query("SELECT * FROM `i_new_product` WHERE brand = '$n'  AND m_cat=1 ") or die(mysqli_error($conn)); 	 
			while($fimp1 = $qimpo->fetch_array()){
				?>
					<div class="product-grid love-grid">
						<div class="more-product-info"><span>TOP</span></div>						
		...
		$qimpo = $conn->query("SELECT * FROM  cat_list
											WHERE cat_list.id='$catd' AND m_cat_as = 1")
											or die(mysqli_error($conn)) ;
												while($fimp1 = $qimpo->fetch_array()){
												//echo $ccid."".$c_id;
												$n =  $fimp1['id'];
		...
		<?php 	
			...
			$qimpo = $conn->query("SELECT * FROM `i_new_product` WHERE cat = '$catd'  AND m_cat=1") or die(mysqli_error($conn)); 	 
			while($fimp1 = $qimpo->fetch_array()){
				?>
					<div class="product-grid love-grid">
							...
							<?php foreach($brand_id as $bbid => $bid){	?>
								...
								<div class="col col-4">
									...
									$qimpo = $conn->query("SELECT * FROM i_new_product, brand_list
											WHERE i_new_product.brand='$bid' AND brand_list.id = i_new_product.brand  AND m_cat=1 ")
											or die(mysqli_error($conn)) ;
												while($fimp1 = $qimpo->fetch_array()){
												$b_id =  $fimp1['id']; 
												echo $b_id;
							...
							<div id="filters">
								<div class="col col-100">
									...
									$qimpo = $conn->query("SELECT * FROM i_new_product, cat_list
											WHERE i_new_product.cat='$c_id' AND cat_list.id = i_new_product.cat  AND m_cat=1")
											or die(mysqli_error($conn)) ;
												while($fimp1 = $qimpo->fetch_array()){
												$cat_id =  $fimp1['id']; 
												//$cat_name[] = $fimp1['name'];
								...
								<i></i><?php 
												$qimpo = $conn->query("SELECT * FROM  cat_list
											WHERE cat_list.id='$cat_id' AND m_cat_as = 1")
											or die(mysqli_error($conn)) ;
												while($fimp1 = $qimpo->fetch_array()){
												//echo $ccid."".$c_id;
												$n =  $fimp1['name'];
